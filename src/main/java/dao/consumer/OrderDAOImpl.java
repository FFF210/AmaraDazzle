...
public class OrderDAOImpl implements OrderDAO {
	...
	// ================== 다시 check
	// 주문 코드 자동 생성
	// * - 오늘 날짜 + 일련번호 형태 (예: 20250915-000001)
	@Override
	...
	public int createOrder(Orders order) throws Exception {
		...
		// useGeneratedKeys="true"로 설정했으므로 order.getOrdersId()로 생성된 ID 확인 가능
		int result = sqlSession.insert("mapper.orders.createOrder", order);
		sqlSession.commit(); // 트랜잭션 커밋
		return result;
	}

	// 주문 상품 추가
	// order_item 테이블에 개별 상품 정보 저장
	@Override
	public int createOrderItem(OrderItem orderItem) throws Exception {
		// OrderMapper.xml의 createOrderItem 쿼리 실행
		int result = sqlSession.insert("mapper.orders.createOrderItem", orderItem);
		sqlSession.commit(); // 트랜잭션 커밋
		return result;
	}
