...
/**
 * Servlet implementation class CancelDetail
 */
@WebServlet("/brand/cancelDetail")
public class CancelDetail extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			// 주문 상세 데이터(Map 구조: summary, items)
			Map<String, Object> cancelOrderDetail = service.cancelOrderDetail(orderId);

			// JSP에 전달
			request.setAttribute("cancelOrderDetail", cancelOrderDetail);
			request.getRequestDispatcher("/brand/cancelDetail.jsp").forward(request, response);
