...
/**
 * Servlet implementation class ExchangeList
 */
@WebServlet("/brand/exchangeList")
public class ExchangeList extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		// 검색 필터
		// searchType = MEMBER(주문자), PRODUCT(상품명), ORDER(주문코드), TRACKINGNO(회수운송장번호)
		...
		try {
			// 서비스 호출
			Map<String, Object> result = service.exchangeOrderListByPage(params);

			// JSP로 전달
			request.setAttribute("exchangeOrderList", result.get("exchangeOrderList"));
			...
			request.getRequestDispatcher("/brand/exchangeList.jsp").forward(request, response);
