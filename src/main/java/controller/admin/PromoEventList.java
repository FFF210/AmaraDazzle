...
@WebServlet("/admin/promoEventList")
public class PromoEventList extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			String startDate2 = request.getParameter("event_startDate");
			String endDate2 = request.getParameter("event_endDate");
			...
			if (!hasCondition) {
				...
				eventCount = event_svc.eventCount(new HashMap<String, String>()); // 전체 카운트
		        paging = m_pg.page_ea(p_no, eventCount); // 페이징Map
		        allEventList = event_svc.allEventList(p_no);
				...
				eventCount = event_svc.eventCount(searchContent);
		        paging = m_pg.page_ea(p_no, eventCount);
		        allEventList = event_svc.searchEventList(searchContent, p_no);
...
//			System.out.println("hasCondition=" + hasCondition + ", sellerNoticeCnt=" + sellerNoticeCnt);
			...
			request.setAttribute("eventCount", eventCount); // 총 게시글 수
			request.setAttribute("eventList", allEventList); // 총 게시물 리스트

			request.getRequestDispatcher("/admin/promotion_event.jsp").forward(request, response);
