...
<c:set var="notifications"
	value="${empty notifications ? '0' : notifications}" />
<c:set var="highlightColor"
	value="${empty highlightColor ? '#9C2EF5' : highlightColor}" />
...
<div class="user-info" style="--ui-accent:${highlightColor}">
	...
	<div class="user-info__top">
		...
		<button class="bell" type="button" aria-label="알림">
			<!-- 단순 SVG 아이콘 (종) -->
			<svg class="bell__icon" width="18" height="18" viewBox="0 0 24 24"
				aria-hidden="true">
        <path
					d="M12 22c1.1 0 2-.9 2-2h-4c0 1.1.9 2 2 2Zm6-6v-5c0-3.07-1.63-5.64-4.5-6.32V4a1.5 1.5 0 1 0-3 0v.68C7.63 5.36 6 7.92 6 11v5l-2 2v1h16v-1l-2-2Z" />
      </svg>
	...
	<div class="user-info__bottom">
		<div class="stat">
			<span class="label">포인트</span> <span class="value"><span
				class="em">${points}</span> p</span>
		</div>
		<div class="stat">
			<span class="label">쿠폰</span> <span class="value"><span
				class="em">${coupons}</span> 개</span>
		</div>
	</div>
</div>
