...
<link rel="stylesheet" href="<c:url value='/consumer/css/ticketDetail.css'/>">
...
<main class="main-content">
    ...
    <div class="ticket-info-box">
        ...
        <div class="info-row">
            ...
            <span class="info-value">
                <fmt:formatDate value="${ticket.questionedAt}" pattern="yyyy-MM-dd HH:mm" />
            </span>
        </div>
    ...
    <div class="content-section">
        ...
        <!-- 첨부 이미지 -->
        <c:if test="${not empty ticket.image1FileId or not empty ticket.image2FileId or not empty ticket.image3FileId}">
            <div class="attached-images">
                <h4>첨부 이미지</h4>
                <div class="image-list">
                    <c:if test="${not empty ticket.image1FileId}">
                        <img src="<c:url value='/file/download?fileId=${ticket.image1FileId}'/>" alt="첨부이미지1" class="attached-img">
                    </c:if>
                    <c:if test="${not empty ticket.image2FileId}">
                        <img src="<c:url value='/file/download?fileId=${ticket.image2FileId}'/>" alt="첨부이미지2" class="attached-img">
                    </c:if>
                    <c:if test="${not empty ticket.image3FileId}">
                        <img src="<c:url value='/file/download?fileId=${ticket.image3FileId}'/>" alt="첨부이미지3" class="attached-img">
                    </c:if>
                </div>
            </div>
        </c:if>
        ...
        <div class="content-section answer-section">
            ...
            <div class="answer-info">
                <span class="answer-date">
                    <fmt:formatDate value="${ticket.answeredAt}" pattern="yyyy-MM-dd HH:mm" /> 답변
                </span>
            </div>
    ...
    <div class="action-button">
        <button type="button" class="btn btn-outline btn-lg" onclick="goBack()">목록으로</button>
    </div>
