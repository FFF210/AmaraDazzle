			...
			<div class="page-breadcrumb">
				<c:choose>
					<c:when test="${param.from eq 'order'}">
						<my:breadcrumb items="주문관리:/brand/orderList,주문상세:/brand/orderDetail?orderId=${param.orderId}&from=${param.from},배송상세" />
					</c:when>
					<c:otherwise>
			...
			<form class="product-form">
				...
				<section class="form-section">
					...
					<div class="form-group tag">
						...
						<c:choose>
							<c:when
								test="${shippingDetail.summary.displayStatus == 'SHIPPING'}">
								<my:tag color="blue" size="lg" text="배송중" />
							</c:when>
							<c:when
								test="${shippingDetail.summary.displayStatus == 'DELIVERED'}">
								<my:tag color="green" size="lg" text="배송완료" />
							</c:when>
							<c:when
								test="${shippingDetail.summary.displayStatus == 'COLLECTING'}">
								<my:tag color="red" size="lg" text="상품 회수중" />
							</c:when>
							<c:when
								test="${shippingDetail.summary.displayStatus == 'EXCHANGE SHIPPING'}">
								<my:tag color="gray" size="lg" text="교환 배송중" />
							</c:when>
						</c:choose>
					</div>
				</section>
				...
				<section class="form-section">
					...
					<div class="table-wrapper">
						<table class="table">
							...
							<tbody>
								<c:forEach var="item" items="${shippingDetail.items}"
									varStatus="loop">
									<tr>
										<td>${loop.index + 1}</td>
