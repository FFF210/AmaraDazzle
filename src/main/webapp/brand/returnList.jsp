	...
	<my:layout>
		<div class="page-container">
			...
			<div class="page-tableFilter">
				<my:tableFilter
					filters="status|주문상태:ALL=전체|REQUESTED=반품요청|APPROVED=반품승인|REFUNDING=환불대기|COMPLETED=횐불완료|REJECTED=반품거절"
					hasDate="true" searchItems="주문번호,주문자,주문상품명,회수운송장번호" />
			</div>
			...
			<div class="page-table">
				<div class="table-wrapper">
					<table class="table">
						...
						<tbody>
							<c:forEach var="order" items="${returnOrderList}"
								varStatus="loop">
								<tr>
									<td><fmt:formatDate value="${order.updatedAt}"
											pattern="yyyy-MM-dd HH:mm" /></td>
									<td><a
										href="/brand/orderDetail?orderId=${order.ordersId}&from=return"><c:out
									...
									<td><c:choose>
											<c:when test="${order.orderStatus == 'RETURN_REQUESTED'}">
												<my:tag color="yellow" size="sm" text="반품요청" />
											</c:when>
											<c:when test="${order.orderStatus == 'RETURN_APPROVED'}">
												<my:tag color="yellow" size="sm" text="반품승인" />
											</c:when>
											<c:when test="${order.orderStatus == 'RETURN_REFUNDING'}">
												<my:tag color="yellow" size="sm" text="환불대기" />
											</c:when>
											<c:when test="${order.orderStatus == 'RETURN_REJECTED'}">
												<my:tag color="red" size="sm" text="반품거절" />
											</c:when>
											<c:when test="${order.orderStatus == 'RETURN_COMPLETED'}">
												<my:tag color="green" size="sm" text="환불완료" />
											</c:when>
										</c:choose>
										</td>
									<td>
			...
			<div class="page-pagination">
				<my:pagination currentPage="${currentPage}"
					...
					baseUrl="/brand/returnList?${queryString}" />
			</div>
		</div>
	</my:layout>
